...
class BasePolicy(ABC):
    ...
    def action_distribution(
        self,
        context: np.ndarray,          # (n_rounds, n_features) или (n_features,)
        base_scores: np.ndarray,      # (n_rounds, n_actions) или (n_actions,)
        candidate_items: np.ndarray | None = None,  # опционально!
    ) -> np.ndarray:
